...
testSets = ["schoenig", "xu2015FOX-AR", "xu2015AAVS1", "xu2015Train", "chari2015Train", "eschstruth", "varshney2015", "ren2015", "farboud2015", "doench2014-Hs", 'chari2015Valid_293T']
# removed:"museumIC50", 
...
def parseAndBinarize(inData):
    ...
    if inData=="schoenig" or inData=="concordet2" or inData=="eschstruth":
        convScores = {1:0, 2:0, 3:1, 4:1}
    # museum scores are inverted
    if inData.startswith("museum"):
        convScores = {1:1, 2:0, 3:0, 4:0}

    labels = np.array([convScores[s] for s in scores])
    #for seq, score in zip(seqs, scores):
        #print seq, score, seq[18:20]
    #print inData, labels
...
def evalAllScores_takeBest(seqs, labels):
    """ create rec/prec/f1 for all main scores, defining positive as 'best X'
    ...
    for scoreType in scoreTypes:
        ...
        seqScoreList = []
        for seq in seqs:
            longSeq = shortToLong[seq]
            scoreDict = seqScores[longSeq]
        ...
        for seq in seqs:
            if seq in posSeqs:
                ...
                predLabels.append(0)
        #print "score, posCount, total", scoreType, posCount, len(labels)
        #print "seqs and scores", seqScoreList
        #print "posSeqs", posSeqs
        #print "seqs", seqs
        #print "labels", predLabels
        typeEval[scoreType] = scorePreds(labels, predLabels)
    return typeEval

def evalAllScores_75Perc(seqs, labels):
    " create rec/prec/f1 for all main scores, defining TP as 'over the 75 percentile' "
    # run scoreCutoffs.py to get these
    cutoffs = {'doench': 0.32398638678651903, 'svm': 0.77276957430125004, 'chariRank': 79.365247038449994, 'ssc': 0.41655549999999997}
...
def evalClassifiers(classifiers, testSets):
    ...
    for dataset in testSets:
        ...
        scoreTypeEvals = evalAllScores_takeBest(seqs, labels)
        for scoreType, (rec, prec, f1) in scoreTypeEvals.iteritems():
            row = [scoreType, dataset, dataSize, posCount, rec, prec, f1]
            rows.append(row)
    return rows
...
def main():
    ...
    headers = ["classifierName", "dataset", "size", "posCount", "recall", "precision", "f1"]
    ofh.write ( "\t".join(headers)+"\n")

    rows = evalClassifiers(classifiers, testSets)

...
def splitXyzVals(xVals, yVals, zVals, zCutoff):
    ...
    x1, y1, z1 = [], [], []
    x2, y2, z2 = [], [], []
    assert(len(xVals)==len(yVals)==len(zVals))
...
def makeBubblePlot(ax, xVals, yVals, areas):
    ...
    print "lowX", lowX
    print "lowY", lowY
    lowZ = np.array([max(0.001, x) for x in lowZ])
    print "s", lowZ
...
def makeTwoSubplots(bubbleSpecScores, bubbleGuideCounts, bubbleAreasSize, mitHistXVals, mitHistYVals, otCountsHistMit, suffix):

    f, axarr = plt.subplots(1, 2)

    # left subplot: scatter plot with bubble sizes
    f.set_size_inches(10,4)
    assert(88 in bubbleSpecScores)
    leg1 = makeBubblePlot(axarr[0], bubbleSpecScores, bubbleGuideCounts, bubbleAreasSize)
    ...
    extraArts = [xlab, ylab]
    if leg1!=None:
        extraArts.append(leg1)
    plt.savefig(plotFname, format = 'pdf', bbox_extra_artists=extraArts, bbox_inches='tight')
    plt.savefig(plotFname.replace(".pdf", ".png"), bbox_extra_artists=extraArts, bbox_inches='tight')
...
def main():
    ...
    # parse the predicted offtargets to get the predicted spec scores
    if not isfile(TMPFNAME):
        print "Generating the spec score cache %s" % TMPFNAME
    ...
    # iterate over guides, merge data and prep
    ofh = open("out/specScoreVsOtCount.tsv", "w")
    headers = ["guide", "CRISPORSpecScore", "MITSpecScore", "strongOtCount", "weakOtCount", "otSum"]
    ...
    for guideName, guideSeq in guideSeqs.iteritems():
        ...
        otSum = otShareSum[guideName]
        row = [guideName, crisporScore, mitScore, weakOtCount, strongOtCount, otSum]

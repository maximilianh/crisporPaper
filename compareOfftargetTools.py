...
def main():
    ...
    #row = [mm, len(mitOts), len(crispOts), len(casOffOts), 
    # len(crispNotMit), len(casOffNotMit), len(mitNotCrisp), len(casOffNotCrisp), 
    # mitMissSeq, crispMissSeqMit, crispMissSeqCasOff, 
    # ",".join(diffCounts)]
    ...
    for mm in range(minMm, maxMm+1):
        ...
        crispNotCasOff = crispOts - casOffOts
        ...
        if len(mitNotCrisp)>0:
            crispMissSeqMit = ",".join(list(mitNotCrisp)[:3])
        else:
            crispMissSeqMit = "None"

        if len(crispNotCasOff)>0:
            casOffMissSeq = ",".join(list(crispNotCasOff)[:3])
        else:
            casOffMissSeq = "None"

...
class HiearchicalHeatmap(object):
    ...
    color_gradient = 'hot'   # Can be: see color_gradients dictionary
...
###############################################################################
class MyHeatmap(HiearchicalHeatmap):
    ...
    def data(self, size=20):
        ...
        df = pandas.read_table("out/effScoreComp.tsv", index_col=0)
        for row in df:
            print repr(row)
        #df['Mary']['Day'] = 1.5
        #df['Issac']['Day'] = 1.0
    ...
    def plot(self):
        ...
        self.path = 'out/' + self. short_name + '.pdf'
        fig, axm, axcb, cb = HiearchicalHeatmap.plot(self)
        cb.set_label("Pearson Correlation")
        pyplot.savefig(self.path)
        pyplot.savefig(self.path.replace(".pdf", ".png"))
        print "saved to %s and .png" % self.path
